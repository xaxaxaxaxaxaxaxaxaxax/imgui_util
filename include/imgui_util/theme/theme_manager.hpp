...
namespace imgui_util::theme {
    ...
    public:
        ...
        [[nodiscard]]
        const theme_config &get_current_theme() const {
            return current_theme_;
        }

        // Preset access
        [[nodiscard]]
        static std::span<const theme_preset> get_presets(); // all built-in presets
        [[nodiscard]]
        static theme_config get_preset(std::string_view name); // build theme_config from named preset
        [[nodiscard]]
        static const theme_preset *find_preset(std::string_view name); // nullptr if not found
