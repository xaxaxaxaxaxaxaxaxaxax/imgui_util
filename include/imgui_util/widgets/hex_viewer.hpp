            ...
            if (const child child_scope{child_id.c_str(), ImVec2(0, 0), ImGuiChildFlags_Borders}) {
                ...
                while (clipper.Step()) {
                    for (int row_i = clipper.DisplayStart; row_i < clipper.DisplayEnd; ++row_i) {
                        ...
                        if (selected_.has_value() && *selected_ >= row_offset
                            && *selected_ < row_offset + bytes_per_row_) {
                        ...
                        for (const auto &[hl_offset, hl_length, hl_color]: highlights_) {
                            ...
                            const float x0 = row_pos.x + addr_w + static_cast<float>(hl_start) * byte_w
                                + group_gaps_before(hl_start);
                            const float x1 = row_pos.x + addr_w + static_cast<float>(hl_end) * byte_w
                                + group_gaps_before(hl_end) - space_w;
                            ...
                            if (col < row_bytes) {
                                ...
                                if (editable && editing_offset_.has_value() && *editing_offset_ == byte_offset) {
                                    ...
                                    if (editable && ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
                                        ...
                                        std::copy_n(tmp.c_str(), std::min(tmp.size() + 1, edit_buf_.size()),
                                                    edit_buf_.data());
                                    }
                                }
                            } else {
